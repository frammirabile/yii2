...
class Migration extends Component implements MigrationInterface
{
    ...
    /**
     * @var string sql to be executed when applying this migration
     */
    protected $up;

    /**
     * @var string sql to be executed when removing this migration
     */
    protected $down;
    ...
    public function down(): bool
    {
        try {
            ...
            if ($this->down !== null)
                $this->execute($this->down);
            elseif (preg_match('/M\d{12}Create(\w+)Table$/', $class = get_class($this), $matches))
                $this->dropTable(Inflector::underscore($matches[1]));
            else {
     ...
     *
     * @return null|bool return a false value to indicate the migration fails
     * and should not proceed further. All other return values mean the migration succeeds.
     */
     ...
     * @param null|string $delete the ON DELETE option. Most DBMS support these options: RESTRICT, CASCADE, NO ACTION, SET DEFAULT, SET NULL
     * @param null|string $update the ON UPDATE option. Most DBMS support these options: RESTRICT, CASCADE, NO ACTION, SET DEFAULT, SET NULL
     * @return void
     * @throws Exception
     */
