...
class Module extends ServiceLocator
{
     ...
     * This namespace will be used to load model classes by prepending it to the model
     * class name.
     *
     * If not set, it will use the `models` sub-namespace under the namespace of this module.
     * For example, if the namespace of this module is `foo\bar`, then the default
     * model namespace would be `foo\bar\models`.
     *
     * See also the [guide section on autoloading](guide:concept-autoloading) to learn more about
     * defining namespaces and how classes are loaded.
     */
     ...
     * @param null|Module $parent the parent module (if any).
     ...
     * given in configuration. The default implementation will initialize [[controllerNamespace]] and [[modelNamespace]]
     * if not set.
     *
    ...
    {
        foreach (['controller', 'model'] as $value)
            if ($this->{$value.'Namespace'} === null) {
                $class = get_class($this);

                if (($pos = strrpos($class, '\\')) !== false)
                    $this->{$value.'Namespace'} = substr($class, 0, $pos).'\\'.$value.'s';
            }
    }
    ...
    /**
     * Returns the directory that contains the model classes according to [[modelNamespace]].
     * Note that in order for this method to return a value, you must define
     * an alias for the root namespace of [[modelNamespace]].
     *
     * @return string the directory that contains the model classes
     * @throws InvalidArgumentException if there is no alias defined for the root namespace of [[modelNamespace]]
     */
    public function getModelPath(): string
    {
        return Yii::getAlias('@'.str_replace('\\', '/', $this->modelNamespace));
    }
     ...
     * @return bool whether the named module exists. Both loaded and unloaded modules are considered.
